...
// Initializes references to other scripts and UI elements
    private void Start(){
...
// Monitors the game state each frame
    void Update(){
        ...
        //this for live demo if (Input.GetKeyDown("e")) EndGame(); // Call EndGame when "E" is pressed
        //this for live demo if (Input.GetKeyDown("w")) WinGame(); // Call WinGame when "W" is pressed
    }

// Fully resets the game state and clears enemies/turrets
    public void ResetGame(bool isFullReset = true){
        // 1. Unpause
        Time.timeScale = 1f; // Reset time scale in case game was paused

        // 2. Stop coroutines FIRST
...
//Ends the game. Triggers game over UI
    void EndGame(){
        ...
        else Debug.LogError("GameOverUI is not assigned or found in the scene.");
        Debug.Log("Game Over!");
        // Pause the game
        Time.timeScale = 0;
    }

//Wins the game. Triggers the win UI
    public void WinGame(){
        ...
        else Debug.LogError("GameWinUI is not assigned or found in the scene.");
        Debug.Log("You Win!");
        // Pause the game
        Time.timeScale = 0;
