...
// Update the text with the number of rounds--
    void OnEnable() => roundsText.text = PlayerStats.Rounds.ToString(); 
    
//Reference in Unity, the onClick event of Continue button in GameWin UI
    public void ContinueGame(){
        // Reset the game state in GameManager
        GameManager gameManager = FindFirstObjectByType<GameManager>();
        if (gameManager != null) gameManager.ContinueGameState(); // Call a method in GameManager to reset the game state
...
//Reference in Unity, the onClick event of NextMap button in GameWin UI
    public void NextMap(){
        ...
        if (currentSceneIndex + 1 >= totalScenes) SceneManager.LoadScene("Map Selection"); // If the current scene is the last one, return to the Map Selection menu
        else SceneManager.LoadScene("Map Selection"); // Load the next scene in the build index
    }
