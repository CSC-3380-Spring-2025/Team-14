...
public class Enemy : MonoBehaviour{
    ...
    private int wavepointIdenx = 0;//incrementing to make sure we have the correct number of waypoint
      ...
      private static int enemiesDestroyed = 0; // Track destroyed enemies
    public static int maxEnemiesBeforeGameOver = 3; // Game ends when 3 are destroyed
    ...
    void GetNextWayPoint(){
        ...
        if(wavepointIdenx >= WayPoint.points.Length - 1){
           ...
           if (enemiesDestroyed == maxEnemiesBeforeGameOver) {
                GameOver();
            }
        ...
        target = WayPoint.points[wavepointIdenx];//new target will be the next index
         void GameOver() {
        Debug.Log(" GAME OVER: 3 enemies crossed to the end!");
